...
const EMBEDDING_PY = path.join(ROOT_DIR, 'lib/embedding.py');
...
if (USE_AUTO_SYNC) {
  import('./supabase_auto_sync.js').then(mod => {
    mod.startAutoSync().then(() => console.log('✅ 自動同步已啟動'))
...
if (!fs.existsSync(CACHE_PATH)) {
  ...
  const supabaseResult = spawnSync('node', [SUPABASE_FETCH], { stdio: 'inherit' });
...
export async function getEmbedding(text) {
  try {
    const py = spawnSync('python', [EMBEDDING_PY, text], { encoding: 'utf-8' });
    if (py.error) {
